function [final_groups] = GenerateSolutionCE(params,Team,probabilites)
    ...
    while size(left_without_team,2) ~= 0 
            ...
            left_without_team = ProjectCE.Get_member_out(left_without_team,1);
        ...
        while team_not_finsih && current_member_to_add_index <= length(left_without_team)
           if ~ProjectCE.MembersCombine(final_groups,left_without_team(current_member_to_add_index),team_index,probabilites)
               current_member_to_add_index = current_member_to_add_index + 1;
               continue;
           end
           if ProjectCE.GroupValidation(Team,final_groups,team_index,left_without_team(current_member_to_add_index))
                final_groups(left_without_team(current_member_to_add_index)) = team_index;
                left_without_team = ProjectCE.Get_member_out(left_without_team,current_member_to_add_index);
           else
                current_member_to_add_index = current_member_to_add_index + 1;
           end
